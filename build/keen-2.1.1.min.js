...
d.draw(a)}}}),Keen.Series.prototype.interval=function(a){return this.attributes.interval=a,this},Keen.showLoading=function(a){(new Spinner).spin(a)},Keen.prettyNumber=function(a){function b(a,c){var a=String(a),d=a.split(".");if(d.length>1){a=d[0];var f=d[1];2==a.length&&f.length>0?f.length>0?a=a+"."+f.charAt(0):a+="0":1==a.length&&f.length>0&&(a=a+"."+f.charAt(0),a+=f.length>1?f.charAt(1):"0")}var g=a.length;return a.split(".").length>1&&g--,3>=g?String(a)+e[c]:b(Number(a)/1e3,c+1)}var c=a.toPrecision(3);if(Number(c)==a&&String(a).length<=4)return String(a);if(a>=1||-1>=a){var d="";0>a&&(a=-a,d="-");var e=["","k","M","B","T"];return d+b(a,0)}return a.toPrecision(3)},Date.prototype.stdTimezoneOffset=function(){var a=new Date(this.getFullYear(),-1,1),b=new Date(this.getFullYear(),6,1);return Math.max(a.getTimezoneOffset(),b.getTimezoneOffset())},Date.prototype.dst=function(){return this.getTimezoneOffset()<this.stdTimezoneOffset()},Keen.loadedCallback&&"function"==typeof Keen.loadedCallback&&Keen.loadedCallback()}();